...
with models.DAG(
    dag_id="jax_nightly_stack_tpu_e2e",
  ...
  for accelerator, slices in axlearn_test_configs.items():
    ...
    for slice_num in slices:
      axlearn_jax_stable_stack_test = config.get_gke_config(
          ...
          time_out_in_min=300,
          run_model_cmds=(
              "./tpu-jobset-multislice-startup.sh"
          ...
          test_name=f"axlearn-jax-nightly-stack-{accelerator}-{slice_num}x",
          # docker_image=DockerImage.AXLEARN_TPU_JAX_STABLE_STACK.value,
          docker_image="us-docker.pkg.dev/supercomputer-testing/axlearn/arc-tpu-py312-jax-nightly:latest",
